    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-2 text-center text-sm text-gray-600">
          Sign in to your account to continue helping and being helped
        </p>
      </div>
                ...
                <div className="flex flex-col space-y-2">
                  <div className="ml-3">
                    <h3 className="text-sm font-medium text-blue-800">
                      Debug: API URL - {process.env.REACT_APP_API_URL || 'http://localhost:5000/api'}
                    </h3>
                  </div>
                  <button
                    type="button"
                    ...
                    className="ml-3 inline-flex items-center px-3 py-1 border border-transparent text-xs font-medium rounded-md text-blue-700 bg-blue-100 hover:bg-blue-200"
                  >
                    Test API Connection
                  </button>
                  <button
                    ...
                    onClick={async () => {
                      try {
                        const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5000/api'}/reset-rate-limit`);
                        const data = await response.json();
                        console.log('Rate limit reset response:', data);
                        alert(`Rate Limit Reset: ${data.status} - ${data.message}`);
                      } catch (error) {
                        console.error('Rate limit reset error:', error);
                        alert('Rate Limit Reset Failed');
                      }
                    }}
                    className="ml-3 inline-flex items-center px-3 py-1 border border-transparent text-xs font-medium rounded-md text-green-700 bg-green-100 hover:bg-green-200"
                  >
                    Reset Rate Limit
                  </button>
                </div>
